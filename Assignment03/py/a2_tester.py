...
def perr(progtype, prog, expectations):
    ...
    if code == 0:
        return
    print(
        f"While trying to start {progtype}, got {['EOF', 'TIMEOUT'][code-1]}")
    print("(expected)\t", HELLO_CLIENT if progtype == 'client' else HELLO_SERVER)
    print(f"(your {progtype})\t", prog.before.decode())
    sys.exit(1)
...
def start_server(arg):
    ...
    """
    CMD = choose_cmd()
    server = px.spawn(f"{CMD} {arg}", timeout=TIMEOUT)
...
def start_client(arg):
    ...
    """
    CMD = choose_cmd()
    client = px.spawn(f"{CMD} {arg} localhost", timeout=TIMEOUT)
...
class Test:
    ...
    def __init__(self, args, print_server, verbose=False):
        self._verbose = verbose
        self._print_server = print_server
        self._netster_args = args
        self._s = start_server(self._netster_args)
        self._c = start_client(self._netster_args)
    ...
    def byehiexit(self):
        ...
        """
        t = [test_client(self._c, "goodbye", "farewell")
             ]  # stop the first client
        c = start_client(self._netster_args)  # start the second client
        t.append(test_client(c, "hi", "hi"))
        t.append(test_client(c, "exit", "ok"))
        if self._print_server:
            ...
            ports = sorted({
                # this will fail if the server logs another 4+ digit number before the port
                re.search("([0-9]{4,})", st, re.M).group(1)
    ...
    def parallel(self):
        ...
        with Pool(4) as p:
            results = [p.apply_async(start_and_test(
                self._netster_args), m) for m in msgs]
            for r in results:
...
def parse_args():
    ...
    c_hello = s.add_parser(
        "hello", help="Send 'hello' from client, expecting 'world' response")
    c_asdf = s.add_parser(
        "asdf", help="Send 'asdf' from client, expecting 'asdf' response")
    c_rand = s.add_parser(
        "rand", help="Send a random word from a list, expect same word in response")
    c_hibye = s.add_parser("hibye", help="Send 'hello' then 'goodbye'")
    c_exit = s.add_parser(
        "exit", help="Send 'exit', which should stop the server")
    c_hiexit = s.add_parser("hiexit", help="Send 'hello', followed by 'exit'")
    c_parallel = s.add_parser(
        "parallel", help="Start multiple clients at the same time")
    c_byehiexit = s.add_parser(
        "byehiexit", help="Send 'goodbye', then say 'hello' then 'exit' from another client")
    c_byehiexit.add_argument(
        "--server", action='store_true', help="Display server output (default: client)")
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    if not args.command:
        s = start_server(netster_args)
        print((s.before+s.after).decode())
        sys.exit(0)

    tst = Test(
        netster_args, args.server if 'server' in args else None, args.verbose)
